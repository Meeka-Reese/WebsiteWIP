#version 300 es
    ...
    void main()
    {
        ...
        float RandDulling = 200.0*(.5+abs(sin(Frame*5.0))); //larger value = less random
        float RandScale1 = ((RandDulling - 1.0) + rand(vec2(Frame + gl_FragCoord.x,Frame + gl_FragCoord.y))) / RandDulling;
        float RandScale2 = ((RandDulling - 1.0) + rand(vec2(Frame* .4,Frame * .6))) / RandDulling;
        vec2 screenSpace = vec2(((gl_FragCoord.x * RandScale1)/((uResolution.x * RandScale2))), 
        ...
        float FinCutoff = abs(sin(Frame * 5.0));
        if (((Comp.r + Comp.g + Comp.b) / 3.0) < FinCutoff) {Comp = vec3(0.0);}
    
        float FlashAm = (10.0 + abs(sin(Frame * 25.0))) / 11.0;
        vec3 Inv = vec3(1.0) - (Comp * FlashAm);
        vec3 Fin = mix(Comp, Inv, -.1 + abs(sin(Frame * 4.0)) * .8);
